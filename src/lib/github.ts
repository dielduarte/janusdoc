...
export function createOctokit(token?: string): Octokit {
  ...
  if (!auth) {
    throw new Error('GitHub token not found. Set GITHUB_TOKEN environment variable or use --token flag.');
  }
...
export function formatComment(result: AnalysisResult): string {
  ...
  let comment = `## 📚 JanusDoc - Documentation Update Suggestions\n\n`;
  ...
  for (const suggestion of result.suggestions) {
    ...
    if (suggestion.relatedCodeFiles.length > 0) {
      comment += `**Related code files:**\n`;
      for (const file of suggestion.relatedCodeFiles) {
        comment += `- \`${file}\`\n`;
      }
      comment += '\n';
    }
...
export async function findExistingComment(
  ...
  const janusDocComment = comments.find(
    comment => comment.body?.includes('## 📚 JanusDoc - Documentation Update Suggestions')
  );
