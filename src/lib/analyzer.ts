...
const SuggestionSchema = z.object({
  ...
  reason: z.string().describe('Why this documentation needs to be updated based on the code changes'),
  suggestedChanges: z.string().describe('Specific suggestions for what to update in the documentation'),
  relatedCodeFiles: z.array(z.string()).describe('List of changed code files that relate to this doc update'),
});
...
const ANALYSIS_PROMPT = `You are a documentation expert. Your task is to analyze code changes and determine which existing documentation files need to be updated.
...
2. Identify which existing documentation files might be affected by these changes
...
5. Only suggest updates for docs that are genuinely affected - don't suggest updates just because a file exists
...
export async function analyzeChanges(
  ...
  if (docs.length === 0) {
    return {
      suggestions: [],
      summary: 'No existing documentation files found to analyze.',
    };
  }

  // Format docs for the prompt
  const docsFormatted = docs
    .map(doc => `### ${doc.path}\n\`\`\`\n${truncateContent(doc.content, 1500)}\n\`\`\``)
    .join('\n\n');
  ...
  const prompt = ANALYSIS_PROMPT
    ...
    .replace('{diff}', truncateContent(diff, 8000))
